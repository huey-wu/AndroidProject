...
//	{ RT5645_GPIO_CTRL2	, 0x0004},
	...
	{ RT5645_MICBIAS	, 0x0018 },
	{ RT5645_GEN_CTRL3	, 0x1380 },
 ...
 */
int rt5645_headset_detect( int jack_insert)
{
	int jack_type, val;
	struct snd_soc_codec *codec = g_rt5645->codec;
	printk("lml in  rt5645_headset_detect111\n");
	if (jack_insert) {
		...
		snd_soc_write(codec, RT5645_JD_CTRL3, 0x00f0);
		...
		printk("val=%d\n",val);
		printk("lml in  rt5645_headset_detect222\n");
...
extern int rt5645_check_jd_status(void)
{
	struct snd_soc_codec *codec = g_rt5645->codec;
...
static const char *rt5645_asrc_mode[] = {"Disable", "Enable"};
static const SOC_ENUM_SINGLE_DECL(rt5645_asrc_enum, 0, 0, rt5645_asrc_mode);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	//DBG("%s\n", __FUNCTION__);
	ucontrol->value.integer.value[0] = rt5645->combo_jack_en;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	rt5645->combo_jack_en = ucontrol->value.integer.value[0];
	if(rt5645->combo_jack_en) {
	
         int i = 0;
         while(i<3)
        	{
	          rt5645_headset_detect(1);
	          msleep(20);
	          i++;
        	} 
	}
...
static int rt5645_asrc_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt5645_priv *rt5645 = snd_soc_codec_get_drvdata(codec);
	...
	//DBG("%s\n", __FUNCTION__);
	ucontrol->value.integer.value[0] = rt5645->asrc_en;

	return 0;
}

static int rt5645_asrc_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt5645_priv *rt5645 = snd_soc_codec_get_drvdata(codec);
	...
	//DBG("%s\n", __FUNCTION__);
	if (rt5645->asrc_en == ucontrol->value.integer.value[0])
		return 0;

	rt5645->asrc_en = ucontrol->value.integer.value[0];
	switch (rt5645->asrc_en) {
	case rt5645_ASRC_DIS://disable ASRC
		...
		snd_soc_write(codec, RT5645_ADDA_CLK1, 0x0014);
	
		break;

	case rt5645_ASRC_EN://enable ASRC
		printk("%s enable\n", __FUNCTION__);
		...
		rt5645_index_write(codec, 0x3d, 0x3e40);
		snd_soc_write(codec, 0x80, 0x4000);
        snd_soc_write(codec, 0x81, 0x0302);
		snd_soc_write(codec, 0x82, 0x0800);
		snd_soc_write(codec, RT5645_ASRC_1, 0x0000);			
		snd_soc_write(codec, RT5645_ASRC_2, 0x0000);
		snd_soc_write(codec, RT5645_ASRC_3, 0x0000);
		snd_soc_write(codec, RT5645_ASRC_8, 0x0000);
		snd_soc_update_bits(codec, RT5645_DIG_MISC, 0x0001, 0x0001); 
		
		snd_soc_write(codec, RT5645_ADDA_CLK1, 0x6610);
		break;
...
static const struct snd_kcontrol_new rt5645_snd_controls[] = {
	...
	SOC_ENUM_EXT("ASRC Switch", rt5645_asrc_enum,
		rt5645_asrc_get, rt5645_asrc_put),
...
static DEVICE_ATTR(codec_reg_adb, 0666, rt5645_codec_adb_show, rt5645_codec_adb_store);
...
#endif
	int ret;
	printk("lml in rt5645_probe33333\n");
...
g_rt5645 = rt5645;
	...
	printk("lml in rt5645_probe end\n");
	return 0;
}
		    ...
		    const struct i2c_device_id *id)
{
	...
	int ret;
	printk("lml in rt5645_i2c_probe111111111\n");
	...
	if (ret < 0)
		kfree(rt5645);
	printk("lml in rt5645_i2c_probe222222222\n");
	return ret;
}
